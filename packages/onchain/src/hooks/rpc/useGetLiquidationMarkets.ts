...
export const useGetLiquidationMarkets = ({ account }: Params) => {
    const { execute, isApiAvailable } = useRpc("liquidation", "getLiquidationMarkets")
    ...
    return useQuery({
        ...
        queryFn: enabled ? async () => {
            ...
            return response.map((item) => ({
                ...
                formatted_user_bid: item.user_bid ? formatUnit(item.user_bid, item.bid_asset_decimals) : null
            }))
        } : skipToken,
