...
const Loading = ({ stage, value, symbol, error, data }: Props) => {
  const stageMessages: Record<TransactionStage, StageMessage> = {
    form: {
      title: "Provide Repay Details",
      description: (
        <div className={styles.description}>
          Please enter the quantity of tokens you wish to Repay to the pool.
        </div>
      ),
    },
    ready: {
      title: "Assets Prepared for Repay",
      description: (
        <div className={styles.description}>
          Your Repay request has been configured
    ...
    wallet: {
      ...
      description: (
        <div className={styles.description}>
          Authorize this transaction in your wallet to proceed with the Repay
          of&nbsp;
          <span className={styles.token}>
            {symbol && <TokenIcon width={16} height={16} symbol={symbol} />}{" "}
            {value} {symbol}
